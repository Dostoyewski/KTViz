...
def build_percent_diag(filename, dist_max, dist_min, step):
    ...
    for i, name in enumerate(names):
        foldername = name.split(sep="\\")[7]
        foldername = foldername.split(sep="_")
        dist = min(float(foldername[1]), float(foldername[2]))
        if codes[i] == 0 or codes[i] == 5 or codes[i] == 1:
            code0[int((dist - dist_min) / step)] += 1
            N_dists[int((dist - dist_min) / step)] += 1
        # elif codes[i] == 1:
        #     code1[int((dist - dist_min) / step)] += 1
        #     N_dists[int((dist - dist_min) / step)] += 1
        elif codes[i] == 2:
            code2[int((dist - dist_min) / step)] += 1
            N_dists[int((dist - dist_min) / step)] += 1
        elif codes[i] == 4:
            code4[int((dist - dist_min) / step)] += 1
            N_dists[int((dist - dist_min) / step)] += 1
        # elif codes[i] == 5:
...
if __name__ == "__main__":
    build_percent_diag('report.xlsx', 12, 5, 0.5)
