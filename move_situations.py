...
def move_point(lat, lon, lat1, lon1, lat2, lon2):
    path = Geodesic.WGS84.Inverse(lat1, lon1, lat, lon)
    path = Geodesic.WGS84.Direct(lat2, lon2, path['azi1'], path['s12'])
...
def move_path(path, lat1, lon1, lat2, lon2, dcog):
    for i, item in enumerate(path['items']):
        ...
        if i == 0:
            end_lat, end_lon = move_point(item['lat'], item['lon'], lat1, lon1, lat2, lon2)

        item['lat'], item['lon'] = end_lat, end_lon
        item['begin_angle'] = (item['begin_angle'] + dcog) % 360
...
def data_move_targets(case_dir, lat1, lon1, lat2, lon2, dcog):
    ...
    for target in target_data:
        target['lat'], target['lon'] = move_point(target['lat'], target['lon'], lat1, lon1, lat2, lon2)
        target['COG'] = (target['COG'] + dcog) % 360
...
def process_table(table, pretty=True):
